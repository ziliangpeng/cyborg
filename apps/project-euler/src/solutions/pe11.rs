    ...
    for row in 0..20i32 {
        for col in 0..20i32 {
            for (dr, dc) in DIRECTIONS {
                // Check if all 4 positions are in bounds
                let end_row = row + 3 * dr;
                let end_col = col + 3 * dc;
                if !(0..20).contains(&end_row) || !(0..20).contains(&end_col) {
                    continue;
                }

                let product: u32 = (0..4)
                    .map(|i| GRID[(row + i * dr) as usize][(col + i * dc) as usize])
                    .product();
...
mod tests {
    ...
    #[test]
    fn test_solve() {
        assert_eq!(solve(false), "70600674");
    }
}
