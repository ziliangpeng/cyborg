...
from crawler import AsyncCrawler, SyncCrawler
...
def main():
    ...
    parser.add_argument(
        ...
        dest="use_async",
        action="store_true",
        help="Use async crawler with aiohttp (default: threading with requests)",
    )
    parser.add_argument("--profile", action="store_true", help="Enable function profiling")
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Enable verbose logging (show each URL being crawled)"
    )
