...
class CyberVisionHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 (stdlib naming)
        ...
        # Add /ui/ route for shared UI files
        if path.startswith("/ui/"):
            ...
            if not safe_path.exists():
                runfiles_path = _MODULE_DIR / "cybervision.runfiles" / "_main" / "libs" / "cybervision-ui" / rel
        ...
        if path.startswith("/shaders/"):
            ...
            if not safe_path.exists():
                ...
                runfiles_path = (
                    _MODULE_DIR / "cybervision.runfiles" / "_main" / "libs" / "cybervision-core" / "shaders" / rel
                )
                if runfiles_path.exists():
                    safe_path = runfiles_path
        ...
        if path.startswith("/lib/"):
            ...
            # For shared lib, check both original location and Bazel runfiles
            if not safe_path.exists():
                runfiles_path = _MODULE_DIR / "cybervision.runfiles" / "_main" / "libs" / "cybervision-core" / rel
                if runfiles_path.exists():
                    safe_path = runfiles_path
            self._send_file(safe_path, allowed_base=None)
            return
        ...
        if path.startswith("/libs/"):
            ...
            if safe_path is None:
                ...
                return
            if not safe_path.exists():
                runfiles_path = _MODULE_DIR / "cybervision.runfiles" / "_main" / "libs" / rel
                if runfiles_path.exists():
                    safe_path = runfiles_path
            self._send_file(safe_path, allowed_base=None)
            return

        # Add /models/ route for ML model files
        ...
        # Add /api/video route for video streaming
        if path == "/api/video":
