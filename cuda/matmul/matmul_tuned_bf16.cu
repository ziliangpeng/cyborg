...
// ============================================================================
// Tuned Configuration - 128x256 block tiles for better N-dimension coalescing
// ============================================================================

#define BM 128
#define BN 256      // Wider N for better global memory coalescing
...
__global__ void convertFP32ToBF16Tuned(const float* input, __nv_bfloat16* output, int size) {
...
// ============================================================================
// Tuned BF16 Kernel with Double Buffering
// ============================================================================

__global__ void __launch_bounds__(BLOCK_SIZE, 2)
matmulTunedBf16Kernel(const __nv_bfloat16* __restrict__ A,
    ...
    // Accumulators - 4x4 WMMA tiles per warp
    ...
    for (int kTile = 0; kTile < numKTiles; kTile++) {
        ...
        // Load all A fragments for this warp
        #pragma unroll
        for (int m = 0; m < WMMA_TILES_M; m++) {
        ...
        // Load all B fragments for this warp
        #pragma unroll
        for (int n = 0; n < WMMA_TILES_N; n++) {
        ...
        // Compute with RLRL pattern for better register reuse
        // Process m=0 and m=2 together, then m=1 and m=3
        #pragma unroll
        for (int m = 0; m < WMMA_TILES_M; m += 2) {
            // Forward direction for even m
            #pragma unroll
            for (int n = 0; n < WMMA_TILES_N; n++) {
                wmma::mma_sync(c_frag[m][n], a_frag[m], b_frag[n], c_frag[m][n]);
            }
            // Reverse direction for odd m
            #pragma unroll
            for (int n = WMMA_TILES_N - 1; n >= 0; n--) {
                wmma::mma_sync(c_frag[m+1][n], a_frag[m+1], b_frag[n], c_frag[m+1][n]);
            }
        }
...
MatmulTunedBf16::MatmulTunedBf16(int N, int blockDim) : N(N) {
    cudaDeviceProp prop;
    cudaGetDeviceProperties(&prop, 0);
    if (prop.major < 8) {
        throw std::runtime_error("Tuned BF16 requires SM 8.0+");
    }

    if (N % 256 != 0) {
        throw std::runtime_error("N must be multiple of 256 for tuned kernel");
    }
...
void MatmulTunedBf16::execute(const float *d_A, const float *d_B, float *d_C) {
    int convThreads = 256;
    int convBlocks = (N * N / 4 + convThreads - 1) / convThreads;
    convertFP32ToBF16Tuned<<<convBlocks, convThreads>>>(d_A, d_A_bf16, N * N);
    convertFP32ToBF16Tuned<<<convBlocks, convThreads>>>(d_B, d_B_bf16, N * N);

    dim3 threads(BLOCK_SIZE);
    dim3 blocks((N + BN - 1) / BN, (N + BM - 1) / BM);

    matmulTunedBf16Kernel<<<blocks, threads>>>(d_A_bf16, d_B_bf16, d_C, N);
    cudaCheckError(cudaGetLastError());
}

MatmulTunedBf16::~MatmulTunedBf16() {
    cudaFree(d_A_bf16);
    cudaFree(d_B_bf16);
}
