...
// 3-stage pipeline + 2x K unroll = process 2 tiles, prefetch 1 stage ahead
// ============================================================================
...
#define NUM_STAGES 3
#define K_PER_STAGE 2  // Process 2 K tiles per stage

// ============================================================================
// FP32 to BF16 Conversion
// ============================================================================

__global__ void convertFP32ToBF16Hybrid(const float* input, __nv_bfloat16* output, int size) {
...
matmulHybridBf16Kernel(const __nv_bfloat16* __restrict__ A,
                       const __nv_bfloat16* __restrict__ B,
                       float* __restrict__ C,
                       int N) {
#if __CUDA_ARCH__ >= 800
    // 3-stage shared memory, each stage holds K_PER_STAGE tiles
    __shared__ __nv_bfloat16 As[NUM_STAGES][K_PER_STAGE][BK][BM + 8];
    __shared__ __nv_bfloat16 Bs[NUM_STAGES][K_PER_STAGE][BK][BN + 8];
    ...
    const int numStageIterations = N / (BK * K_PER_STAGE);

    // Helper to load a stage
    auto loadStage = [&](int stage, int kBase) {
        #pragma unroll
        for (int u = 0; u < K_PER_STAGE; u++) {
    ...
    for (int s = 0; s < NUM_STAGES - 1; s++) {
        if (s < numStageIterations) {
            loadStage(s, s * BK * K_PER_STAGE);
        }
    }
    ...
    // Main loop
    for (int stageIter = 0; stageIter < numStageIterations; stageIter++) {
        int readStage = stageIter % NUM_STAGES;
        int writeStage = (stageIter + NUM_STAGES - 1) % NUM_STAGES;

        // Load next stage (overlapped with compute)
        if (stageIter + NUM_STAGES - 1 < numStageIterations) {
            loadStage(writeStage, (stageIter + NUM_STAGES - 1) * BK * K_PER_STAGE);
        }

        // Compute on current stage (K_PER_STAGE tiles)
        #pragma unroll
        for (int u = 0; u < K_PER_STAGE; u++) {
            ...
            for (int m = 0; m < WMMA_TILES_M; m++) {
                ...
                wmma::load_matrix_sync(a_frag[m], &As[readStage][u][0][aRow], BM + 8);
            }
            ...
            for (int n = 0; n < WMMA_TILES_N; n++) {
                ...
                wmma::load_matrix_sync(b_frag[n], &Bs[readStage][u][0][bCol], BN + 8);
            }
...
MatmulHybridBf16::MatmulHybridBf16(int N, int blockDim) : N(N) {
    cudaDeviceProp prop;
    cudaGetDeviceProperties(&prop, 0);
    if (prop.major < 8) {
        throw std::runtime_error("Hybrid BF16 requires SM 8.0+");
    }
...
void MatmulHybridBf16::execute(const float *d_A, const float *d_B, float *d_C) {
    int convThreads = 256;
    int convBlocks = (N * N / 4 + convThreads - 1) / convThreads;
    convertFP32ToBF16Hybrid<<<convBlocks, convThreads>>>(d_A, d_A_bf16, N * N);
    convertFP32ToBF16Hybrid<<<convBlocks, convThreads>>>(d_B, d_B_bf16, N * N);

    dim3 threads(BLOCK_SIZE);
    dim3 blocks((N + BN - 1) / BN, (N + BM - 1) / BM);

    matmulHybridBf16Kernel<<<blocks, threads>>>(d_A_bf16, d_B_bf16, d_C, N);
    cudaCheckError(cudaGetLastError());
}

MatmulHybridBf16::~MatmulHybridBf16() {
    cudaFree(d_A_bf16);
    cudaFree(d_B_bf16);
}
