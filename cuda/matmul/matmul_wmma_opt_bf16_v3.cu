...
// ============================================================================
// Configuration - Smaller BK for better occupancy
// ============================================================================

#define BM 128          // Block tile M
#define BN 128          // Block tile N
#define BK 16           // Small K tile for less shared memory

#define WM 32           // Warp tile M (smaller for more parallelism)
...
__global__ void convertFP32ToBF16OptV3(const float* input, __nv_bfloat16* output, int size) {
...
// ============================================================================
// Optimized BF16 WMMA Kernel V3
// ============================================================================

__global__ void __launch_bounds__(BLOCK_SIZE, 2)  // Request 2 blocks per SM
matmulWmmaOptBf16V3Kernel(const __nv_bfloat16* __restrict__ A,
                          const __nv_bfloat16* __restrict__ B,
                          float* __restrict__ C,
                          int N) {
#if __CUDA_ARCH__ >= 800
    // Smaller shared memory for better occupancy
    ...
    // Accumulators - each warp computes 2x4 WMMA tiles
    ...
    // Load config: A = 128*16 = 2048, B = 16*128 = 2048
    // 256 threads -> 8 elements per thread each
    ...
    for (int kTile = 0; kTile < numKTiles; kTile++) {
        ...
        // Compute - just one WMMA_K step since BK = WMMA_K = 16
...
MatmulWmmaOptBf16V3::MatmulWmmaOptBf16V3(int N, int blockDim) : N(N) {
    cudaDeviceProp prop;
...
void MatmulWmmaOptBf16V3::execute(const float *d_A, const float *d_B, float *d_C) {
    int convThreads = 256;
    int convBlocks = (N * N / 4 + convThreads - 1) / convThreads;
    convertFP32ToBF16OptV3<<<convBlocks, convThreads>>>(d_A, d_A_bf16, N * N);
    convertFP32ToBF16OptV3<<<convBlocks, convThreads>>>(d_B, d_B_bf16, N * N);

    dim3 threads(BLOCK_SIZE);
    dim3 blocks((N + BN - 1) / BN, (N + BM - 1) / BM);

    matmulWmmaOptBf16V3Kernel<<<blocks, threads>>>(d_A_bf16, d_B_bf16, d_C, N);
    cudaCheckError(cudaGetLastError());
}

MatmulWmmaOptBf16V3::~MatmulWmmaOptBf16V3() {
    cudaFree(d_A_bf16);
    cudaFree(d_B_bf16);
}
