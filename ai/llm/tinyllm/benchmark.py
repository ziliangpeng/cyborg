...
def _run_once(model, input_ids: Tensor, output_len: int, temperature: float, top_k, use_kv_cache: str = "none") -> dict:
    ...
    generate(model, input_ids, max_new_tokens=1, temperature=temperature, top_k=top_k, kv_cache=kv_cache)
    ...
    generate(model, input_ids, max_new_tokens=output_len, temperature=temperature, top_k=top_k, kv_cache=kv_cache2)
...
@click.option(
    "--kv-cache",
    type=click.Choice(["none", "simple", "variable"]),
    default="none",
    help="KV cache implementation: none, simple, variable",
)
def main(model, warmup, runs, input_lens, output_lens, temperature, top_k, kv_cache):
    ...
    click.echo(f"Model loaded in {time.perf_counter() - t0:.2f}s ({format_param_count(llm.param_count())} params)")
    click.echo(f"warmup={warmup}  runs={runs}  kv_cache={kv_cache}  input_lens={in_lens}  output_lens={out_lens}")
    click.echo()

    configs = [(i, o) for i in in_lens for o in out_lens if i + o <= llm.config.n_positions]
    ...
    click.echo(
        f"  {'input':>6}  {'output':>6}  metric    {'mean':>8}  {'median':>8}  {'p95':>8}  {'min':>8}  {'max':>8}  unit"
    )
    click.echo("-" * 80)
    ...
    for in_len, out_len in configs:
        ...
        results = [_run_once(llm, ids, out_len, temperature, top_k, use_kv_cache=kv_cache) for _ in range(runs)]
